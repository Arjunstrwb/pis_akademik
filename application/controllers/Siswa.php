<?php
	...
	class Siswa extends CI_Controller
	{
		...
		$namanya = $_GET['nama'];
		$nisnnya = $_GET['nisn'];
		$data["get_data"] = $this->model_siswa->report_nilai($nisnnya);
		...
		// loadDataSiswa() -> untuk menampilkan data siswa nisn dan nama sesuai kode_kelas yang dipilih di filter, lalu ditampilkan ke div id = kelas yang bedada di view/siswa_aktif -> terletak di controller Siswa
		  ...
		  function import(){
		    ...
		    foreach($sheet as $row){
		      ...
		      if($numrow > 1){
		        ...
		        $nisn = $get[0]; // Ambil data NIS dari kolom A di csv
		...
		// 	redirect('siswa_aktif');
		//   } else {
		// 	$nisn	= $this->uri->segment(3);
		// 	$data['nisn'] = $this->db->get_where('tbl_nilai', array('nisn' => $nisn))->row_array();
		// 	$this->template->load('template', 'nilai/add_nilai', $data);
		//   }
		// }
		...
		// 	$this->db->where('nisn', $kelas);
		// 	$siswa = $this->db->get('tbl_siswa');
		// 	foreach ($siswa->result() as $row) {
		// 		echo "<tr>
		// 				<td class='text-center'>$row->nisn</td>
		// 				<td>$row->nama</td>
		// 				<td class='text-center'>".anchor('siswa/nilai_add/'.$row->nisn, '<button class="btn bg-navy btn-flat margin">Tambah Data</button>')."</td>
		// 			 </tr>";
		// 	}
		// 	echo "</table>";
		// }
